...
function needMaskingText(node2, maskTextClass, maskTextSelector, checkAncestors) {
	...
	try {
		...
		} else if (classMatchesRegex$1(el, maskTextClass, checkAncestors)) return true;
		if (maskTextSelector) {
			if (checkAncestors) {
...
var parser$1 = /* @__PURE__ */ function() {
	...
	_proto.colon = function colon(tokens) {
		...
		for (var _iterator = _create_for_of_iterator_helper_loose(tokens.entries()), _step; !(_step = _iterator()).done;) {
			var _step_value = _step.value, i2 = _step_value[0];
			token = _step_value[1];
			type = token[0];
			if (type === "(") brackets += 1;
			if (type === ")") brackets -= 1;
...
function fromJSON$1$1(json, inputs) {
	...
	var ownInputs = json.inputs, defaults = _object_without_properties_loose(json, ["inputs"]);
	if (ownInputs) {
		inputs = [];
	...
	if (defaults.source) {
		var _defaults_source = defaults.source, inputId = _defaults_source.inputId;
...
var require$$2$2 = /* @__PURE__ */ getAugmentedNamespace$2(/* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
	__proto__: null,
	default: {}
}, Symbol.toStringTag, { value: "Module" })));
...
function stringify$4$2(node2, builder) {
	new Stringifier$1$2(builder).stringify(node2);
}
...
var Node$4$2 = /* @__PURE__ */ function() {
	...
	_proto.markDirty = function markDirty() {
		if (this[isClean$2$2]) {
			this[isClean$2$2] = false;
			var next = this;
			while (next = next.parent) next[isClean$2$2] = false;
		}
	};
	_proto.next = function next() {
		if (!this.parent) return void 0;
...
var nanoid$1$2 = function(size) {
	...
	var id = "";
	var i2 = size;
	while (i2--) id += urlAlphabet$2[Math.random() * 64 | 0];
	return id;
};
...
var PreviousMap$2$2 = /* @__PURE__ */ function() {
	...
	_proto.consumer = function consumer() {
		if (!this.consumerCache) this.consumerCache = new SourceMapConsumer$2$2(this.text);
		return this.consumerCache;
	};
	_proto.decodeInline = function decodeInline(text) {
		var baseCharsetUri = /^data:application\/json;charset=utf-?8;base64,/;
		var baseUri = /^data:application\/json;base64,/;
		if (/^data:application\/json;charset=utf-?8,/.test(text) || /^data:application\/json,/.test(text)) return decodeURIComponent(text.substr(RegExp.lastMatch.length));
		if (baseCharsetUri.test(text) || baseUri.test(text)) return fromBase64$2(text.substr(RegExp.lastMatch.length));
		var encoding = text.match(/data:application\/json;([^,]+),/)[1];
		throw new Error("Unsupported source map encoding " + encoding);
	};
	_proto.getAnnotationURL = function getAnnotationURL(sourceMapString) {
	...
	_proto.loadMap = function loadMap(file, prev) {
		...
		} else if (_instanceof(prev, SourceMapConsumer$2$2)) return SourceMapGenerator$2$2.fromSourceMap(prev).toString();
		else if (_instanceof(prev, SourceMapGenerator$2$2)) return prev.toString();
		else if (this.isMap(prev)) return JSON.stringify(prev);
		else throw new Error("Unsupported previous source map format: " + prev.toString());
		else if (this.inline) return this.decodeInline(this.annotation);
		else if (this.annotation) {
...
var Input$4$2 = /* @__PURE__ */ function() {
	function Input2(css, opts) {
		...
		if (opts.from) if (!pathAvailable$1$2 || /^\w+:\/\//.test(opts.from) || isAbsolute$2(opts.from)) this.file = opts.from;
		else this.file = resolve$1$2(opts.from);
		...
		if (!this.file) this.id = "<input css " + nanoid$3(6) + ">";
		if (this.map) this.map.file = this.from;
	}
	...
	_proto.error = function error(message, line, column, opts) {
		...
		var origin = this.origin(line, column, endLine, endColumn);
		if (origin) result2 = new CssSyntaxError$1$2(message, origin.endLine === void 0 ? origin.line : {
			column: origin.column,
			line: origin.line
		}, origin.endLine === void 0 ? origin.column : {
			column: origin.endColumn,
		...
		else result2 = new CssSyntaxError$1$2(message, endLine === void 0 ? line : {
			column,
			line
		}, endLine === void 0 ? column : {
			column: endColumn,
		...
		if (this.file) {
			if (pathToFileURL$1$2) result2.input.url = pathToFileURL$1$2(this.file).toString();
			result2.input.file = this.file;
		}
		return result2;
	};
	_proto.fromOffset = function fromOffset(offset) {
		...
		} else lineToIndex = this[fromOffsetCache$2];
		lastLine = lineToIndex[lineToIndex.length - 1];
		var min = 0;
		if (offset >= lastLine) min = lineToIndex.length - 1;
		else {
	...
	_proto.origin = function origin(line, column, endLine, endColumn) {
		...
		var fromUrl;
		if (isAbsolute$2(from.source)) fromUrl = pathToFileURL$1$2(from.source);
		else fromUrl = new URL(from.source, this.map.consumer().sourceRoot || pathToFileURL$1$2(this.map.mapFile));
		var result2 = {
			column: from.column,
			endColumn: to && to.column,
			endLine: to && to.line,
		...
		if (fromUrl.protocol === "file:") if (fileURLToPath$2) result2.file = fileURLToPath$2(fromUrl);
		else throw new Error("file: protocol is not available in this PostCSS build");
		var source = consumer.sourceContentFor(from.source);
		if (source) result2.source = source;
		return result2;
...
if (terminalHighlight$3 && terminalHighlight$3.registerInput) terminalHighlight$3.registerInput(Input$4$2);
...
var mapGenerator$2 = /* @__PURE__ */ function() {
	...
	_proto.generate = function generate() {
		this.clearAnnotation();
		if (pathAvailable$3 && sourceMapAvailable$3 && this.isMap()) return this.generateMap();
		else {
			var result2 = "";
			this.stringify(this.root, function(i2) {
				result2 += i2;
	...
	_proto.generateMap = function generateMap() {
		if (this.root) this.generateString();
			...
			var prev = this.previous()[0].consumer();
			prev.file = this.outputFile();
			this.map = SourceMapGenerator$4.fromSourceMap(prev, { ignoreInvalidMapping: true });
		} else {
	...
	_proto.path = function path(file) {
		...
		if (typeof this.mapOpts.annotation === "string") from = dirname$3(resolve$3(from, this.mapOpts.annotation));
...
var Comment$4$2 = /* @__PURE__ */ function(Node$2$2) {
	...
	function Comment2(defaults) {
		var _this = Node$2$2.call(this, defaults) || this;
		_this.type = "comment";
		return _this;
	}
	return Comment2;
...
var Container$7$2 = /* @__PURE__ */ function(Node$1$2) {
	...
	_proto.normalize = function normalize(nodes, sample) {
		var _this = this;
		if (typeof nodes === "string") nodes = cleanSource$2(parse$4$2(nodes).nodes);
		else if (typeof nodes === "undefined") nodes = [];
		else if (Array.isArray(nodes)) {
			nodes = nodes.slice(0);
			for (var _iterator = _create_for_of_iterator_helper_loose(nodes), _step; !(_step = _iterator()).done;) {
		...
		} else if (nodes.selector) nodes = [new Rule$4$2(nodes)];
		else if (nodes.name) nodes = [new AtRule$4$2(nodes)];
		else if (nodes.text) nodes = [new Comment$3$2(nodes)];
		else throw new Error("Unknown node type in node creation");
		return nodes.map(function(i2) {
...
Container$7$2.rebuild = function(node2) {
	if (node2.type === "atrule") Object.setPrototypeOf(node2, AtRule$4$2.prototype);
	else if (node2.type === "rule") Object.setPrototypeOf(node2, Rule$4$2.prototype);
	else if (node2.type === "decl") Object.setPrototypeOf(node2, Declaration$3$2.prototype);
	else if (node2.type === "comment") Object.setPrototypeOf(node2, Comment$3$2.prototype);
	else if (node2.type === "root") Object.setPrototypeOf(node2, Root$6$2.prototype);
	node2[my$1$2] = true;
	if (node2.nodes) node2.nodes.forEach(function(child) {
...
var Document$3$2 = /* @__PURE__ */ function(Container$6$2) {
	...
	function Document23(defaults) {
		var _this = Container$6$2.call(this, _extends({ type: "document" }, defaults)) || this;
		if (!_this.nodes) _this.nodes = [];
		return _this;
	}
	...
	_proto.toResult = function toResult(opts) {
		if (opts === void 0) opts = {};
		return new LazyResult$4$2(new Processor$3$2(), this, opts).stringify();
	};
	return Document23;
}(Container$6$2);
		...
		switch (code) {
			...
			case OPEN_PARENTHESES$2:
				...
				if (prev === "url" && n2 !== SINGLE_QUOTE$2 && n2 !== DOUBLE_QUOTE$2 && n2 !== SPACE$2 && n2 !== NEWLINE$2 && n2 !== TAB$2 && n2 !== FEED$2 && n2 !== CR$2) {
					...
					next = css.indexOf(")", pos + 1);
					content = css.slice(pos, next + 1);
					if (next === -1 || RE_BAD_BRACKET$2.test(content)) currentToken = [
						"(",
						"(",
						pos
					];
			...
			case AT$2:
				...
				if (RE_AT_END$2.lastIndex === 0) next = css.length - 1;
				else next = RE_AT_END$2.lastIndex - 2;
				currentToken = [
					"at-word",
					css.slice(pos, next + 1),
					pos,
			...
			case BACKSLASH$2:
				...
				if (escape && code !== SLASH$2 && code !== SPACE$2 && code !== NEWLINE$2 && code !== TAB$2 && code !== CR$2 && code !== FEED$2) {
					...
					if (RE_HEX_ESCAPE$2.test(css.charAt(next))) {
						while (RE_HEX_ESCAPE$2.test(css.charAt(next + 1))) next += 1;
						if (css.charCodeAt(next + 1) === SPACE$2) next += 1;
					}
				}
				currentToken = [
					"word",
			...
			default:
				if (code === SLASH$2 && css.charCodeAt(pos + 1) === ASTERISK$2) {
					...
					if (RE_WORD_END$2.lastIndex === 0) next = css.length - 1;
					else next = RE_WORD_END$2.lastIndex - 2;
					currentToken = [
						"word",
						css.slice(pos, next + 1),
						pos,
...
var AtRule$3$2 = /* @__PURE__ */ function(Container$5$2) {
	...
	function AtRule2(defaults) {
		var _this = Container$5$2.call(this, defaults) || this;
		_this.type = "atrule";
		return _this;
	}
	...
	_proto.append = function append() {
		...
		if (!this.proxyOf.nodes) this.nodes = [];
		return (_Container$5$2_prototype_append = Container$5$2.prototype.append).call.apply(_Container$5$2_prototype_append, [].concat([this], children));
	};
	_proto.prepend = function prepend() {
		for (var _len = arguments.length, children = new Array(_len), _key = 0; _key < _len; _key++) children[_key] = arguments[_key];
		var _Container$5$2_prototype_prepend;
		if (!this.proxyOf.nodes) this.nodes = [];
		return (_Container$5$2_prototype_prepend = Container$5$2.prototype.prepend).call.apply(_Container$5$2_prototype_prepend, [].concat([this], children));
	};
	return AtRule2;
}(Container$5$2);
...
var Root$5$2 = /* @__PURE__ */ function(Container$4$2) {
	...
	function Root2(defaults) {
		var _this = Container$4$2.call(this, defaults) || this;
		_this.type = "root";
		if (!_this.nodes) _this.nodes = [];
		return _this;
	}
	...
	_proto.toResult = function toResult(opts) {
		if (opts === void 0) opts = {};
		return new LazyResult$3$2(new Processor$2$2(), this, opts).stringify();
	};
	return Root2;
}(Container$4$2);
...
var Rule$3$2 = /* @__PURE__ */ function(Container$3$2) {
	...
	function Rule2(defaults) {
		var _this = Container$3$2.call(this, defaults) || this;
		_this.type = "rule";
		if (!_this.nodes) _this.nodes = [];
		return _this;
	}
	...
	_proto.raw = function raw(node2, prop, tokens, customProperty) {
		...
		for (var i2 = 0; i2 < length; i2 += 1) {
			...
			if (type === "space" && i2 === length - 1 && !customProperty) clean = false;
				...
				prev = tokens[i2 - 1] ? tokens[i2 - 1][0] : "empty";
				next = tokens[i2 + 1] ? tokens[i2 + 1][0] : "empty";
				if (!SAFE_COMMENT_NEIGHBOR$2[prev] && !SAFE_COMMENT_NEIGHBOR$2[next]) if (value.slice(-1) === ",") clean = false;
				else value += token[1];
				else clean = false;
			} else value += token[1];
		}
...
function parse$3$2(css, opts) {
	var parser2 = new Parser22(new Input$2$2(css, opts));
	try {
		parser2.parse();
	} catch (e2) {
		if (e2.name === "CssSyntaxError" && opts && opts.from) {
...
var LazyResult$2$2 = /* @__PURE__ */ function() {
	function LazyResult2(processor2, css, opts) {
		...
		this.processed = false;
		var root2;
		if ((typeof css === "undefined" ? "undefined" : _type_of(css)) === "object" && css !== null && (css.type === "root" || css.type === "document")) root2 = cleanMarks$2(css);
			...
			if (root2 && !root2[my$4]) Container$1$2.rebuild(root2);
		}
	...
	_proto.prepareVisitors = function prepareVisitors() {
		...
		for (var _iterator = _create_for_of_iterator_helper_loose(this.plugins), _step; !(_step = _iterator()).done;) {
			var plugin22 = _step.value;
			if ((typeof plugin22 === "undefined" ? "undefined" : _type_of(plugin22)) === "object") for (var event in plugin22) {
				if (!PLUGIN_PROPS$2[event] && /^[A-Z]/.test(event)) throw new Error("Unknown event " + event + " in " + plugin22.postcssPlugin + ". Try to update PostCSS (" + this.processor.version + " now).");
			...
			return _ts_generator(this, function(_state) {
				switch (_state.label) {
					...
					case 1:
						...
						plugin22 = _this.plugins[i2];
						promise = _this.runOnRoot(plugin22);
						if (!isPromise$2(promise)) return [3, 5];
						_state.label = 2;
					case 2:
						_state.trys.push([
							2,
					...
					case 6:
						...
						_state.label = 7;
					case 7:
						if (!!root2[isClean$4]) return [3, 14];
						root2[isClean$4] = true;
						stack = [toStack$2(root2)];
						_state.label = 8;
					case 8:
						if (!(stack.length > 0)) return [3, 13];
						promise1 = _this.visitTick(stack);
						if (!isPromise$2(promise1)) return [3, 12];
						_state.label = 9;
					case 9:
						_state.trys.push([
							9,
		...
		try {
			if ((typeof plugin22 === "undefined" ? "undefined" : _type_of(plugin22)) === "object" && plugin22.Once) {
				if (this.result.root.type === "document") {
					...
					if (isPromise$2(roots[0])) return Promise.all(roots);
					return roots;
				}
				return plugin22.Once(this.result.root, this.helpers);
			} else if (typeof plugin22 === "function") return plugin22(this.result.root, this.result);
	...
	_proto.stringify = function stringify() {
		...
		if (opts.stringifier) str = opts.stringifier;
		if (str.stringify) str = str.stringify;
		var data = new MapGenerator$1$2(str, this.result.root, this.result.opts).generate();
		this.result.css = data[0];
		this.result.map = data[1];
		return this.result;
	};
	_proto.sync = function sync() {
		...
		for (var _iterator = _create_for_of_iterator_helper_loose(this.plugins), _step; !(_step = _iterator()).done;) {
			var plugin22 = _step.value;
			if (isPromise$2(this.runOnRoot(plugin22))) throw this.getAsyncError();
		}
		this.prepareVisitors();
		if (this.hasListener) {
			var root2 = this.result.root;
	...
	_proto.then = function then(onFulfilled, onRejected) {
		if (!("from" in this.opts)) warnOnce$1$2("Without `from` option PostCSS could generate wrong source map and will not find Browserslist config. Set it to CSS file path or to `undefined` to prevent this warning.");
		return this.async().then(onFulfilled, onRejected);
	};
	_proto.toString = function toString() {
		return this.css;
	...
	_proto.visitSync = function visitSync(visitors, node2) {
		for (var _iterator = _create_for_of_iterator_helper_loose(visitors), _step; !(_step = _iterator()).done;) {
			...
			if (node2.type !== "root" && node2.type !== "document" && !node2.parent) return true;
			if (isPromise$2(promise)) throw this.getAsyncError();
		}
	};
	_proto.visitTick = function visitTick(stack) {
		var visit2 = stack[stack.length - 1];
	...
	_proto.walkSync = function walkSync(node2) {
		var _this = this;
		node2[isClean$4] = true;
		for (var _iterator = _create_for_of_iterator_helper_loose(getEvents$2(node2)), _step; !(_step = _iterator()).done;) {
			var event = _step.value;
			if (event === CHILDREN$2) {
				if (node2.nodes) node2.each(function(child) {
...
var Processor$1$2 = /* @__PURE__ */ function() {
	...
	_proto.process = function process1(css, opts) {
		...
		else return new LazyResult$1$2(this, css, opts);
	};
	_proto.use = function use(plugin22) {
		this.plugins = this.plugins.concat(this.normalize([plugin22]));
		return this;
...
function fromJSON$1$2(json, inputs) {
	...
	if (ownInputs) {
		...
		for (var _iterator = _create_for_of_iterator_helper_loose(ownInputs), _step; !(_step = _iterator()).done;) {
			var input2 = _step.value;
			var inputHydrated = _extends({}, input2, { __proto__: Input$1$2.prototype });
	...
	if (defaults.type === "root") return new Root$1$2(defaults);
	else if (defaults.type === "decl") return new Declaration$1$2(defaults);
	else if (defaults.type === "rule") return new Rule$1$2(defaults);
	else if (defaults.type === "comment") return new Comment$1$2(defaults);
	else if (defaults.type === "atrule") return new AtRule$1$2(defaults);
	else throw new Error("Unknown node type: " + json.type);
}
...
function getUntaintedMethod$2(key, instance, method) {
	...
	var untaintedMethod = getUntaintedPrototype$2(key)[method];
	if (typeof untaintedMethod !== "function") return instance[method];
...
function closestElementOfNode(node2) {
	...
	return node2.nodeType === node2.ELEMENT_NODE ? node2 : index$2.parentElement(node2);
}
function isBlocked(node2, blockClass, blockSelector, checkAncestors) {
	...
	try {
		...
		} else if (classMatchesRegex$1(el, blockClass, checkAncestors)) return true;
	} catch (e2) {}
	if (blockSelector) {
	...
	function MutationBuffer() {
		...
		*/ __publicField$1(this, "addedSet", /* @__PURE__ */ new Set());
		...
		__publicField$1(this, "emit", function() {
			...
			var pushAdd = function(n2) {
				...
				var sn = serializeNodeWithId(n2, {
					...
					onSerialize: function(currentN) {
						...
						if (hasShadowRoot(n2)) _this.shadowDomManager.addShadowRoot(index$2.shadowRoot(n2), _this.doc);
					},
			...
			while (_this.mapRemoves.length) {
				...
				if (removedNode.nodeName === "IFRAME") try {
					_this.iframeManager.removeIframe(removedNode);
				} catch (e2) {}
				else _this.stylesheetManager.cleanupStylesheetsForRemovedNode(removedNode);
				_this.mirror.removeNodeFromMap(removedNode);
			}
			for (var _iterator = _create_for_of_iterator_helper_loose(_this.movedSet), _step; !(_step = _iterator()).done;) {
				var n2 = _step.value;
				if (isParentRemoved(_this.removesSubTreeCache, n2, _this.mirror) && !_this.movedSet.has(index$2.parentNode(n2))) continue;
				pushAdd(n2);
			}
		...
		* */ __publicField$1(this, "genAdds", function(n2, target) {
			...
			if (!isBlocked(n2, _this.blockClass, _this.blockSelector, false)) {
				...
				if (hasShadowRoot(n2)) index$2.childNodes(index$2.shadowRoot(n2)).forEach(function(childN) {
...
function removeMutationBufferForDoc(doc) {
	for (var i2 = mutationBuffers.length - 1; i2 >= 0; i2--) if (mutationBuffers[i2].getDoc() === doc) mutationBuffers.splice(i2, 1);
}
...
function initInputObserver(param) {
	...
	function eventHandler(event) {
		...
		if (target && tagName === "OPTION") target = index$2.parentElement(target);
...
function initCustomElementObserver(param) {
	...
	return patch$2(win.customElements, "define", function(original) {
...
var IframeManager = /* @__PURE__ */ function() {
	...
	_proto.attachIframe = function attachIframe(iframeEl, childSn) {
		...
		if (iframeEl.contentDocument) this.iframeContentDocumentMap.set(iframeEl, iframeEl.contentDocument);
	...
	*/ _proto.cleanupStylesheetsForRemovedNode = function cleanupStylesheetsForRemovedNode(removedNode) {
		...
		try {
			if (removedNode.nodeType === Node.DOCUMENT_NODE) {
				var doc = removedNode;
				if (doc.adoptedStyleSheets) for (var _iterator = _create_for_of_iterator_helper_loose(doc.adoptedStyleSheets), _step; !(_step = _iterator()).done;) {
					var sheet = _step.value;
			...
			if (removedNode.nodeName === "STYLE") {
				...
				if (styleEl.sheet) this.styleMirror.remove(styleEl.sheet);
			}
			if (removedNode.nodeName === "LINK" && removedNode.rel === "stylesheet") {
				var linkEl = removedNode;
				if (linkEl.sheet) this.styleMirror.remove(linkEl.sheet);
			}
			if (removedNode.childNodes) removedNode.childNodes.forEach(function(child) {
	...
	takeFullSnapshot$1 = function(isCheckout) {
		...
		var node2 = snapshot(document, {
			...
			onSerialize: function(n2) {
				...
				if (hasShadowRoot(n2)) shadowDomManager.addShadowRoot(index$2.shadowRoot(n2), document);
			},
			onIframeLoad: function(iframe, childSn) {
...
function classMatchesRegex(node2, regex, checkAncestors) {
	if (!node2) return false;
	if (node2.nodeType !== node2.ELEMENT_NODE) return classMatchesRegex(index.parentNode(node2), regex);
	for (var eIndex = node2.classList.length; eIndex--;) {
		var className = node2.classList[eIndex];
		if (regex.test(className)) return true;
	}
...
function getAugmentedNamespace(n) {
	...
	if (typeof f == "function") {
		var a = function a2() {
			if (_instanceof(this, a2)) return Reflect.construct(f, arguments, this.constructor);
...
var CssSyntaxError$3 = /* @__PURE__ */ function(Error1) {
	...
	function CssSyntaxError(message, line, column, source, file, plugin2) {
		...
		if (file) _this.file = file;
		if (source) _this.source = source;
		if (plugin2) _this.plugin = plugin2;
		if (typeof line !== "undefined" && typeof column !== "undefined") if (typeof line === "number") {
			_this.line = line;
			_this.column = column;
		} else {
...
var Node$4 = /* @__PURE__ */ function() {
	...
	_proto.positionInside = function positionInside(index2, stringRepresentation) {
		...
		var column = this.source.start.column;
		var line = this.source.start.line;
		for (var i = 0; i < index2; i++) if (string[i] === "\n") {
			column = 1;
			line += 1;
		} else column += 1;
		return {
	...
	_proto.toJSON = function toJSON(_, inputs) {
		...
		for (var name in this) {
			...
			if (Array.isArray(value)) fixed[name] = value.map(function(i) {
				if ((typeof i === "undefined" ? "undefined" : _type_of(i)) === "object" && i.toJSON) return i.toJSON(null, inputs);
				else return i;
			});
			else if ((typeof value === "undefined" ? "undefined" : _type_of(value)) === "object" && value.toJSON) fixed[name] = value.toJSON(null, inputs);
			else if (name === "source") {
...
var mapGenerator = /* @__PURE__ */ function() {
	...
	_proto.path = function path(file) {
		...
		if (typeof this.mapOpts.annotation === "string") from = dirname(resolve$4(from, this.mapOpts.annotation));
...
var Container$7 = /* @__PURE__ */ function(Node$1) {
			...
			get: function get(node2, prop) {
				if (prop === "proxyOf") return node2;
					...
					return (_node2 = node2)[prop].apply(_node2, [].concat(args.map(function(i) {
						if (typeof i === "function") return function(child, index2) {
				...
				else if (prop === "nodes") return node2.nodes.map(function(i) {
					return i.toProxy();
				});
				else if (prop === "first" || prop === "last") return node2[prop].toProxy();
				else return node2[prop];
	...
	_proto.walkAtRules = function walkAtRules(name, callback) {
		...
		if (_instanceof(name, RegExp)) return this.walk(function(child, i) {
	...
	_proto.walkDecls = function walkDecls(prop, callback) {
		...
		if (_instanceof(prop, RegExp)) return this.walk(function(child, i) {
	...
	_proto.walkRules = function walkRules(selector, callback) {
		...
		if (_instanceof(selector, RegExp)) return this.walk(function(child, i) {
...
var parser = /* @__PURE__ */ function() {
	...
	_proto.colon = function colon(tokens) {
		...
		var token, type, prev;
		for (var _iterator = _create_for_of_iterator_helper_loose(tokens.entries()), _step; !(_step = _iterator()).done;) {
			var _step_value = _step.value, i = _step_value[0];
			token = _step_value[1];
			type = token[0];
			if (type === "(") brackets += 1;
			if (type === ")") brackets -= 1;
	...
	_proto.stringFrom = function stringFrom(tokens, from) {
		var result2 = "";
		for (var i = from; i < tokens.length; i++) result2 += tokens[i][1];
		tokens.splice(from, tokens.length - from);
		return result2;
	};
	_proto.unclosedBlock = function unclosedBlock() {
...
function parse$3(css, opts) {
	var parser2 = new Parser2(new Input$2(css, opts));
	try {
		parser2.parse();
	} catch (e) {
...
var LazyResult$2 = /* @__PURE__ */ function() {
	function LazyResult(processor2, css, opts) {
		...
		this.plugins = this.processor.plugins.map(function(plugin2) {
			if ((typeof plugin2 === "undefined" ? "undefined" : _type_of(plugin2)) === "object" && plugin2.prepare) return _extends({}, plugin2, plugin2.prepare(_this.result));
			else return plugin2;
		});
	}
	var _proto = LazyResult.prototype;
	...
	_proto.prepareVisitors = function prepareVisitors() {
		...
		var add = function(plugin2, type, cb) {
			if (!_this.listeners[type]) _this.listeners[type] = [];
			_this.listeners[type].push([plugin2, cb]);
		};
		for (var _iterator = _create_for_of_iterator_helper_loose(this.plugins), _step; !(_step = _iterator()).done;) {
			var plugin2 = _step.value;
			if ((typeof plugin2 === "undefined" ? "undefined" : _type_of(plugin2)) === "object") for (var event in plugin2) {
				...
				if (!NOT_VISITORS[event]) {
					if (_type_of(plugin2[event]) === "object") for (var filter in plugin2[event]) if (filter === "*") add(plugin2, event, plugin2[event][filter]);
			...
			return _ts_generator(this, function(_state) {
				switch (_state.label) {
					case 0:
						...
						_state.label = 1;
					case 1:
						if (!(i < _this.plugins.length)) return [3, 6];
	...
	_proto.sync = function sync() {
		...
		for (var _iterator = _create_for_of_iterator_helper_loose(this.plugins), _step; !(_step = _iterator()).done;) {
			var plugin2 = _step.value;
			if (isPromise(this.runOnRoot(plugin2))) throw this.getAsyncError();
		}
		this.prepareVisitors();
		if (this.hasListener) {
			var root2 = this.result.root;
...
var NoWorkResult$1 = /* @__PURE__ */ function() {
	...
	_proto.then = function then(onFulfilled, onRejected) {
		if (!("from" in this._opts)) warnOnce2("Without `from` option PostCSS could generate wrong source map and will not find Browserslist config. Set it to CSS file path or to `undefined` to prevent this warning.");
		return this.async().then(onFulfilled, onRejected);
	};
	_proto.toString = function toString() {
		return this._css;
...
var Processor$1 = /* @__PURE__ */ function() {
	...
	_proto.normalize = function normalize(plugins) {
		...
		for (var _iterator = _create_for_of_iterator_helper_loose(plugins), _step; !(_step = _iterator()).done;) {
			...
			if ((typeof i === "undefined" ? "undefined" : _type_of(i)) === "object" && Array.isArray(i.plugins)) normalized = normalized.concat(i.plugins);
			else if ((typeof i === "undefined" ? "undefined" : _type_of(i)) === "object" && i.postcssPlugin) normalized.push(i);
			else if (typeof i === "function") normalized.push(i);
			else if ((typeof i === "undefined" ? "undefined" : _type_of(i)) === "object" && (i.parse || i.stringify)) throw new Error("PostCSS syntaxes cannot be used as plugins. Instead, please use one of the syntax/parser/stringifier options as outlined in your PostCSS runner documentation.");
...
function fromJSON$1(json, inputs) {
	...
	if (ownInputs) {
		...
		for (var _iterator = _create_for_of_iterator_helper_loose(ownInputs), _step; !(_step = _iterator()).done;) {
			...
			if (inputHydrated.map) inputHydrated.map = _extends({}, inputHydrated.map, { __proto__: PreviousMap2.prototype });
			inputs.push(inputHydrated);
		}
	}
...
function pathToSelector(node) {
	...
	while (node.parentElement) {
		...
		var domSiblings = [];
		if (parent.children && parent.children.length > 0) for (var i = 0; i < parent.children.length; i++) {
...
function isObjTooDeep(obj, limit) {
	...
	for (var _iterator = _create_for_of_iterator_helper_loose(Object.keys(obj)), _step; !(_step = _iterator()).done;) {
		var key = _step.value;
		if (isObject(obj[key]) && isObjTooDeep(obj[key], limit - 1)) return true;
	}
	return false;
}
function stringify(obj, stringifyOptions) {
	...
	return JSON.stringify(obj, function(key, value) {
		...
		if ((typeof value === "undefined" ? "undefined" : _type_of(value)) === "bigint") return value.toString() + "n";
		if (_instanceof(value, Event)) {
			...
			if (_instanceof(value, HTMLElement)) return value ? value.outerHTML : "";
			return value.nodeName;
		} else if (_instanceof(value, Error)) return value.stack ? value.stack + "\nEnd of stack for Error object" : value.name + ": " + value.message;
		return value;
	});
...
function initLogObserver(cb, win, options) {
	...
	if (logOptions.level.includes("error")) {
		var errorHandler = function(event) {
			...
			cb({
				level: "error",
				trace: ErrorStackParser.parse(error).map(function(stackFrame) {
	...
	function replace(_logger, level) {
		var _this = this;
		if (!_logger[level]) return function() {};
		return patch(_logger, level, function(original) {
			...
			return function() {
				...
				inStack = true;
				try {
					var trace = ErrorStackParser.parse(/* @__PURE__ */ new Error()).map(function(stackFrame) {
						return stackFrame.toString();
					}).splice(1);
					var payload = (level === "assert" ? args.slice(1) : args).map(function(s) {
					...
					original.apply(void 0, [].concat(["rrweb logger error:", error], args));
				} finally {
					inStack = false;
				}
			};
...
_.getQueryParam = function(url, param) {
	...
	var regexS = "[\\?&]" + param + "=([^&#]*)", results = new RegExp(regexS).exec(url);
...
_.dom_query = (function() {
	...
	return function(query) {
		if (_.isElement(query)) return [query];
...
_.info = {
	...
	browser: function(user_agent, vendor, opera) {
		...
		else if (_.includes(user_agent, "SamsungBrowser/")) return "Samsung Internet";
		...
		else if (_.includes(user_agent, "Whale/")) return "Whale Browser";
...
var RequestQueue = function(storageKey, options) {
	...
	if (options.enqueueThrottleMs) this.enqueuePersisted = batchedThrottle(_.bind(this._enqueuePersisted, this), options.enqueueThrottleMs);
	else this.enqueuePersisted = _.bind(function(queueEntry) {
...
RequestQueue.prototype.fillBatch = function(batchSize) {
	...
	if (this.usePersistence && batch.length < batchSize) return this.ensureInit().then(_.bind(function() {
		return this.readFromStorage();
	}, this)).then(_.bind(function(storedQueue) {
...
RequestQueue.prototype.removeItemsByID = function(ids) {
	...
	if (!this.usePersistence) return PromisePolyfill.resolve(true);
		...
		return this.lock.withLock(removeFromStorage, this.pid).catch(_.bind(function(err) {
			this.reportError("Error acquiring storage lock", err);
			if (!localStorageSupported(this.lock.storage, true)) return removeFromStorage().then(_.bind(function(success) {
...
RequestQueue.prototype.updatePayloads = function(itemsToUpdate) {
	...
	if (!this.usePersistence) return PromisePolyfill.resolve(true);
	else return this.lock.withLock(_.bind(function lockAcquired() {
		return this.ensureInit().then(_.bind(function() {
			return this.readFromStorage();
		}, this)).then(_.bind(function(storedQueue) {
...
RequestQueue.prototype.clear = function() {
	this.memQueue = [];
	if (this.usePersistence) return this.ensureInit().then(_.bind(function() {
...
RequestBatcher.prototype.scheduleFlush = function(flushMS) {
	...
	if (!this.stopped) this.timeoutID = setTimeout(_.bind(function() {
...
RequestBatcher.prototype.flush = function(options) {
	...
	return this.queue.fillBatch(currentBatchSize).then(_.bind(function(batch) {
		...
		_.each(batch, function(item) {
			...
			if (payload) {
				if (payload["event"] && payload["properties"]) payload["properties"] = _.extend({}, payload["properties"], { "mp_sent_by_lib_version": Config.LIB_VERSION });
				var addPayload = true;
				var itemId = item["id"];
				if (itemId) {
		...
		var batchSendCallback = _.bind(function(res) {
			...
			try {
				if (options.unloading) return this.queue.updatePayloads(transformedItems);
					...
					return PromisePolyfill.resolve();
				} else if (_.isObject(res) && res.httpStatusCode === 413) if (batch.length > 1) {
...
function getPropertiesFromElement(el, ev, blockAttrsSet, extraAttrs, allowElementCallback, allowSelectors) {
	...
	if (shouldTrackElementDetails(el, ev, allowElementCallback, allowSelectors)) _.each(TRACKED_ATTRS.concat(extraAttrs), function(attr) {
...
function getSafeText(el, ev, allowElementCallback, allowSelectors) {
	var elText = "";
	if (shouldTrackElementDetails(el, ev, allowElementCallback, allowSelectors) && el.childNodes && el.childNodes.length) _.each(el.childNodes, function(child) {
		if (isTextNode(child) && child.textContent) elText += _.trim(child.textContent).split(/(\s+)/).filter(shouldTrackValue).join("").replace(/[\r\n]/g, " ").replace(/[ ]+/g, " ").substring(0, 255);
	});
...
function elementLooksSensitive(el) {
	...
	if (typeof name === "string") {
		if (/^cc|cardnum|ccnum|creditcard|csc|cvc|cvv|exp|pass|pwd|routing|seccode|securitycode|securitynum|socialsec|socsec|ssn/i.test(name.replace(/[^a-zA-Z0-9]/g, ""))) return true;
	}
	return false;
}
function shouldTrackElementDetails(el, ev, allowElementCallback, allowSelectors) {
	...
	if (elementLooksSensitive(el)) return false;
	return true;
}
function shouldTrackValue(value) {
	if (value === null || _.isUndefined(value)) return false;
...
function getPrivacyConfig(mixpanelInstance) {
	...
	if (maskAllInputs !== void 0) privacyConfig.input.maskAll = maskAllInputs;
	...
	if (legacyMaskTextClass) if (legacyMaskTextClass instanceof RegExp) privacyConfig.text._legacyClassRegex = legacyMaskTextClass;
	else {
		var classSelector = "." + legacyMaskTextClass;
		if (textMaskingSelectors.indexOf(classSelector) === -1) textMaskingSelectors.push(classSelector);
	}
	...
	if (maskAllText === void 0 && maskTextSelector !== void 0) privacyConfig.text.maskAll = false;
	else if (maskAllText !== void 0) privacyConfig.text.maskAll = maskAllText;
...
function shouldMaskInput(element, privacyConfig) {
	...
	if (ALWAYS_MASKED_INPUT_TYPES.indexOf(inputType) !== -1) return true;
	var autocomplete = (element.getAttribute("autocomplete") || "").toLowerCase();
	if (autocomplete && autocomplete !== "" && autocomplete !== "off") return true;
	if (element.hasAttribute(RRWEB_PASSWORD_ATTRIBUTE)) return true;
	if (elementLooksSensitive(element)) return true;
	if (privacyConfig.input.maskAll) {
		if (elementMatchesSelector(element, privacyConfig.input.unmaskingSelector)) return false;
		return true;
	} else {
		if (elementMatchesSelector(element, privacyConfig.input.maskingSelector)) return true;
		if (elementMatchesSelector(element, COMMON_MASK_CLASSES_SELECTOR)) return true;
		return false;
	}
}
/**
...
function shouldMaskText(element, privacyConfig) {
	...
	if (privacyConfig.text._legacyClassRegex) {
		if (classMatchesRegex(element, privacyConfig.text._legacyClassRegex)) return true;
	}
	if (privacyConfig.text.maskAll) {
		if (elementMatchesSelector(element, privacyConfig.text.unmaskingSelector)) return false;
		return true;
	} else {
		if (elementMatchesSelector(element, privacyConfig.text.maskingSelector)) return true;
		if (elementMatchesSelector(element, COMMON_MASK_CLASSES_SELECTOR)) return true;
		return false;
	}
}
...
SessionRecording.prototype.unloadPersistedData = function() {
	...
	return this.queueStorage.init().catch(function() {
		...
		if (this.getDurationMs() < this._getRecordMinMs()) return this.queueStorage.removeItem(this.batcherKey);
	...
	try {
		this._stopRecording = this._rrwebRecord({
			"emit": function(ev) {
				...
				if (this.lastEventTimestamp === null || ev.timestamp > this.lastEventTimestamp) this.lastEventTimestamp = ev.timestamp;
			}.bind(this),
			...
			"sampling": { "canvas": 15 },
			"plugins": this.getConfig("record_console") ? [getRecordConsolePlugin({ stringifyOptions: {
...
SessionRecording.deserialize = function(serializedRecording, options) {
	return new SessionRecording(_.extend({}, options, {
...
SessionRecording.prototype.getDurationMs = function() {
	if (this.replayStartTime === null) return 0;
	if (this.lastEventTimestamp === null) return (/* @__PURE__ */ new Date()).getTime() - this.replayStartTime;
...
var jsonLogic = /* @__PURE__ */ getDefaultExportFromCjs(requireLogic());
/**
...
var lowercaseJson = function(obj, lowercaseKeys) {
	...
	else if (Array.isArray(obj)) return obj.map(function(item) {
...
var eventMatchesCriteria = function(eventName, properties, criteria) {
	if (eventName !== criteria.event_name) return { matches: false };
	var propertyFilters = criteria.property_filters;
	var filtersMatch = true;
	if (propertyFilters && !_.isEmptyObject(propertyFilters)) try {
...
RageClickTracker.prototype.isRageClick = function(event, options) {
	...
	if (options["interactive_elements_only"] || DEFAULT_RAGE_CLICK_INTERACTIVE_ELEMENTS_ONLY) {
		var target = getClickEventTargetElement(event);
		if (!target || isDefinitelyNonInteractive(target)) return false;
	}
...
Autocapture.prototype.getFullConfig = function() {
	...
	else if (_.isObject(autocaptureConfig)) return _.extend({}, CONFIG_DEFAULTS$1, autocaptureConfig);
...
Autocapture.prototype.initPageviewTracking = function() {
	...
	if (tracked) previousTrackedUrl = _.info.currentUrl();
...
Autocapture.prototype.initRageClickTracking = function() {
	...
	this.listenerRageClick = function(ev) {
		...
		if (this.currentUrlBlocked()) return;
		if (this._rageClickTracker.isRageClick(ev, currentRageClickConfig)) this.trackDomEvent(ev, MP_EV_RAGE_CLICK);
	}.bind(this);
...
var getTargetingPromise = function(loadExtraBundle, targetingSrc) {
	if (win[TARGETING_GLOBAL_NAME] && typeof win[TARGETING_GLOBAL_NAME].then === "function") return win[TARGETING_GLOBAL_NAME];
...
FeatureFlagManager.prototype.getFullConfig = function() {
	...
	if (!ffConfig) return {};
	else if (_.isObject(ffConfig)) return _.extend({}, CONFIG_DEFAULTS, ffConfig);
...
FeatureFlagManager.prototype.updateContext = function(newContext, options) {
	...
	var ffConfig = this.getMpConfig(FLAGS_CONFIG_KEY);
	if (!_.isObject(ffConfig)) ffConfig = {};
		...
		return response.json().then(function(responseBody) {
			...
			_.each(responseFlags, function(data, key) {
				...
				_.each(this.activatedFirstTimeEvents, function(activated, eventKey) {
					if (eventKey.startsWith(prefix)) hasActivatedEvent = true;
				});
				if (hasActivatedEvent) {
					var currentFlag = this.flags && this.flags.get(key);
					if (currentFlag) flags.set(key, currentFlag);
				} else flags.set(key, {
			...
			if (topLevelDefinitions && topLevelDefinitions.length > 0) _.each(topLevelDefinitions, function(def) {
				var flagKey = def["flag_key"];
				var eventKey = getPendingEventKey(flagKey, def["first_time_event_hash"]);
				if (this.activatedFirstTimeEvents[eventKey]) return;
			...
			if (this.activatedFirstTimeEvents) _.each(this.activatedFirstTimeEvents, function(activated, eventKey) {
				var flagKey = getFlagKeyFromPendingEventKey(eventKey);
				if (activated && !flags.has(flagKey) && this.flags && this.flags.has(flagKey)) flags.set(flagKey, this.flags.get(flagKey));
			}, this);
			this.flags = flags;
...
FeatureFlagManager.prototype._loadTargetingIfNeeded = function() {
	...
	_.each(this.pendingFirstTimeEvents, function(evt) {
		if (evt["property_filters"] && !_.isEmptyObject(evt["property_filters"])) hasPropertyFilters = true;
	});
	if (hasPropertyFilters) this.getTargeting().then(function() {
...
FeatureFlagManager.prototype.getTargeting = function() {
	return getTargetingPromise(this.loadExtraBundle.bind(this), this.targetingSrc).catch(function(error) {
...
FeatureFlagManager.prototype.checkFirstTimeEvents = function(eventName, properties) {
	if (!this.pendingFirstTimeEvents || _.isEmptyObject(this.pendingFirstTimeEvents)) return;
	if (win[TARGETING_GLOBAL_NAME] && _.isFunction(win[TARGETING_GLOBAL_NAME].then)) win[TARGETING_GLOBAL_NAME].then(function(library) {
	...
	else this._processFirstTimeEventCheck(eventName, properties, null);
};
/**
...
FeatureFlagManager.prototype._processFirstTimeEventCheck = function(eventName, properties, targeting) {
	_.each(this.pendingFirstTimeEvents, function(pendingEvent, eventKey) {
		...
		if (!targeting) matchResult = {
			...
			matchResult = targeting["eventMatchesCriteria"](eventName, properties, criteria);
		}
		...
		this.recordFirstTimeEvent(pendingEvent["flag_id"], pendingEvent["project_id"], pendingEvent["first_time_event_hash"]);
	}, this);
};
...
var apiActions = {
	...
	union_action: function(list_name, values) {
		...
		if (_.isObject(list_name)) _.each(list_name, function(v, k) {
			if (!this._is_reserved_property(k)) $union[k] = _.isArray(v) ? v : [v];
		}, this);
		else $union[list_name] = _.isArray(values) ? values : [values];
...
MixpanelPeople.prototype._send_request = function(data, callback) {
	...
	if (!this._identify_called()) {
		this._enqueue(data);
		if (!_.isUndefined(callback)) if (this._get_config("verbose")) callback({
			status: -1,
			error: null
		});
		else callback(-1);
		...
		_.each(q_data, function(v, k) {
			if (_.isArray(v)) {
				...
				_.each(v, function(item) {
					if (!_.include(union_q[k], item)) union_q[k].push(item);
				});
			}
		});
		this._pop_from_people_queue(UNSET_ACTION, q_data);
...
MixpanelPersistence.prototype._pop_from_people_queue = function(queue, data) {
	...
	if (!_.isUndefined(q)) _.each(data, function(v, k) {
		if (queue === APPEND_ACTION || queue === REMOVE_ACTION) _.each(q, function(queued_action) {
...
var create_mplib = function(token, config, name) {
	...
	win.dispatchEvent(new win.CustomEvent("$mp_sdk_to_extension_event", { "detail": {
		"instance": instance,
		"source": source,
		"token": token,
		"name": name,
...
MixpanelLib.prototype._init = function(token, config, name) {
	...
	if (this._batch_requests) if (!_.localStorage.is_supported(true) || !USE_XHR) {
	...
	if (mode === SETTING_STRICT || mode === SETTING_FALLBACK) this._fetch_remote_settings(mode).then(_.bind(function() {
		this._check_and_start_session_recording();
	}, this));
	else this._check_and_start_session_recording();
};
/**
...
MixpanelLib.prototype._init_tab_id = function() {
	...
	else if (_.sessionStorage.is_supported()) try {
		var key_suffix = this.get_config("name") + "_" + this.get_config("token");
		var tab_id_key = "mp_tab_id_" + key_suffix;
		var should_generate_new_tab_id_key = "mp_gen_new_tab_id_" + key_suffix;
		if (_.sessionStorage.get(should_generate_new_tab_id_key) || !_.sessionStorage.get(tab_id_key)) _.sessionStorage.set(tab_id_key, "$tab-" + _.UUID());
...
MixpanelLib.prototype._check_and_start_session_recording = addOptOutCheckMixpanelLib(function(force_start) {
	...
	var loadRecorder = _.bind(function(startNewIfInactive) {
		...
		if (_.isUndefined(win[RECORDER_GLOBAL_NAME])) load_extra_bundle(this.get_config("recorder_src"), handleLoadedRecorder);
...
MixpanelLib.prototype.get_session_replay_url = function() {
	var replay_url = null;
	var replay_id = this._get_session_replay_id();
	if (replay_id) replay_url = "https://mixpanel.com/projects/replay-redirect?" + _.HTTPBuildQuery({
		"replay_id": replay_id,
...
MixpanelLib.prototype._set_default_superprops = function() {
	...
	if (this.get_config("store_google") && !this.get_config("stop_utm_persistence")) this.register(_.info.campaignParams());
...
MixpanelLib.prototype._fetch_remote_settings = function(mode) {
	var disableRecordingIfStrict = function() {
		if (mode === "strict") self.set_config({ "record_sessions_percent": 0 });
	};
	...
	var fetchOptions = {
		"method": "GET",
		"headers": { "Authorization": "Basic " + btoa(self.get_config("token") + ":") },
	...
	return win["fetch"](full_url, fetchOptions).then(function(response) {
		...
		if (result && result["sdk_config"] && result["sdk_config"]["config"]) {
			...
			_.each(remote_config, function(value, key) {
				if (DEFAULT_CONFIG.hasOwnProperty(key)) valid_config[key] = value;
			});
			if (_.isEmptyObject(valid_config)) {
				console$1.critical("No valid config keys found in remote settings.");
				disableRecordingIfStrict();
			} else self.set_config(valid_config);
		} else disableRecordingIfStrict();
	}).catch(function(err) {
		clearTimeout(timeout_id);
...
MixpanelLib.prototype._execute_array = function(array) {
	...
	_.each(array, function(item) {
		if (item) {
			fn_name = item[0];
			if (_.isArray(fn_name)) tracking_calls.push(item);
...
MixpanelLib.prototype._track_or_batch = function(options, callback) {
	...
	var send_request_immediately = _.bind(function() {
		if (!send_request_options.skip_hooks) {
			...
			if (truncated_data) truncated_data = truncated_data[0];
		}
		if (truncated_data) {
...
MixpanelLib.prototype.track = addOptOutCheckMixpanelLib(function(event_name, properties, options, callback) {
	...
	properties = _.extend({}, _.info.properties({ "mp_loader": this.get_config("mp_loader") }), marketing_properties, this["persistence"].properties(), this.unpersisted_superprops, this.get_session_recording_properties(), properties);
	...
	if (this.flags && this.flags.checkFirstTimeEvents) this.flags.checkFirstTimeEvents(event_name, properties);
	return ret;
});
/**
...
MixpanelLib.prototype.set_group = addOptOutCheckMixpanelLib(function(group_key, group_ids, callback) {
	if (!_.isArray(group_ids)) group_ids = [group_ids];
	var prop = {};
...
var options_for_register = function(days_or_options) {
	...
	else if (!_.isUndefined(days_or_options)) options = { "days": days_or_options };
	else options = {};
...
MixpanelLib.prototype.set_config = function(config) {
	if (_.isObject(config)) {
		...
		if (config["batch_size"]) _.each(this.request_batchers, function(batcher) {
		...
		if (_.isObject(config["hooks"])) {
			...
			_.each(config["hooks"], function(hook_value, hook_name) {
				if (_.isFunction(hook_value)) this.hooks[hook_name] = [hook_value];
				else if (_.isArray(hook_value)) {
					this.hooks[hook_name] = [];
					for (var i = 0; i < hook_value.length; i++) {
						if (!_.isFunction(hook_value[i])) console$1.critical("Invalid hook added. Hook is not a function");
						this.hooks[hook_name].push(hook_value[i]);
					}
				} else console$1.critical("Invalid hooks added. Ensure that the hook values passed into config.hooks are functions or arrays of functions.");
			}, this);
		}
	}
};
...
MixpanelLib.prototype._gdpr_init = function() {
	if (this.get_config("opt_out_tracking_persistence_type") === "localStorage" && _.localStorage.is_supported()) {
...
MixpanelLib.prototype.opt_in_tracking = function(options) {
	options = _.extend({ "enable_persistence": true }, options);
...
MixpanelLib.prototype.add_hook = function(hook_name, hook_fn) {
	if (!this.hooks[hook_name]) this.hooks[hook_name] = [];
...
MixpanelLib.prototype.remove_hook = function(hook_name, hook_fn) {
	...
	if (this.hooks[hook_name]) {
		...
		if (fn_index !== -1) this.hooks[hook_name].splice(fn_index, 1);
		else console$1.log("remove_hook failed. Matching hook was not found");
	}
};
